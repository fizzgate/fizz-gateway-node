...
- 全球智能节点调度：多区域集群部署，自动选择最优 AI 节点路径，实现跨区域、跨供应商的高可用访问。
- 多渠道与多账号负载均衡：支持多账号池、供应商策略权重、失败自动切换，确保请求稳定不中断。
- 精细化计费与配额体系：兼容余额计费、Token 计费、次卡计费；支持调用明细、成本跟踪、使用记录可视化。
- Agent 智能体能力：原生支持 MCP 工具调用、对话级记忆管理、外部知识库检索。
